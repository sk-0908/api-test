	...
	tests := []struct {
		...
		},
		{
			...
			setupMock: func(mockRepo *MockItemRepository) {
				...
				updatedItem, _ := entity.NewItem("時計1", "時計", "新しいブランド", 1000000, "2023-01-01")
				updatedItem.ID = 1
				mockRepo.On("Update", mock.Anything, int64(1), mock.AnythingOfType("*entity.Item")).Return(updatedItem, nil)
			},
			expectError: false,
			validate: func(t *testing.T, item *entity.Item) {
				assert.Equal(t, "時計1", item.Name)
		...
		},
		{
			...
			validate: func(t *testing.T, item *entity.Item) {
				...
				assert.Equal(t, "時計", item.Category) // 変更されていないことを確認
			},
		},
		{
			name: "異常系: 存在しないアイテム",
		...
		},
		{
			...
			setupMock: func(mockRepo *MockItemRepository) {
				...
				updatedItem, _ := entity.NewItem("時計1", "時計", "ROLEX", 0, "2023-01-01")
				updatedItem.ID = 1
				mockRepo.On("Update", mock.Anything, int64(1), mock.AnythingOfType("*entity.Item")).Return(updatedItem, nil)
			},
			expectError: false,
			validate: func(t *testing.T, item *entity.Item) {
				assert.Equal(t, 0, item.PurchasePrice)
			},
		},
	}
